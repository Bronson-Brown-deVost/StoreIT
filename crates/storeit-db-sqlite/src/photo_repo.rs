...
impl PhotoRepository for SqlitePhotoRepository {
    async fn create(&self, input: CreatePhoto, storage_key: String) -> Result<Photo> {
        ...
        sqlx::query!(
            r#"INSERT INTO photos (id, entity_type, entity_id, storage_key, mime_type, is_primary, rotation)
               VALUES (?, ?, ?, ?, ?, 0, 0)"#,
            id_str,
            entity_type_str,
            entity_id_str,
    ...
    async fn get(&self, id: Uuid) -> Result<Option<Photo>> {
        ...
        let row = sqlx::query!(
            r#"SELECT id, entity_type, entity_id, storage_key, mime_type, is_primary, rotation, created_at
    ...
    async fn insert_raw(&self, photo: &Photo) -> Result<()> {
        ...
        sqlx::query!(
            r#"INSERT INTO photos (id, entity_type, entity_id, storage_key, mime_type, is_primary, rotation, created_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)"#,
            id_str,
            et_str,
            eid_str,
