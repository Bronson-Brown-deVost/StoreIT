...
use storeit_domain::storage::{ImageStorage, StoredImageKeys};
...
/// Legacy: derive thumbnail key from original key (for backward compat with old data).
fn legacy_thumbnail_key(original_key: &str) -> String {
...
const LARGE_MAX_DIM: u32 = 1200;

/// Generate a lossy WebP large display image (~1200px). Returns None if decoding fails.
fn generate_large(data: &[u8]) -> Option<Vec<u8>> {
...
impl ImageStorage for FsImageStorage {
    async fn store(&self, data: &[u8], mime_type: &str) -> Result<StoredImageKeys> {
        ...
        // Generate thumbnail — content-addressable (best-effort)
        let mut result_thumb_key = None;
        if let Some(thumb_data) = generate_thumbnail(&store_data) {
            let thumb_k = compute_key(&thumb_data, "image/webp");
            let thumb_path = self.full_path(&thumb_k);
            ...
            result_thumb_key = Some(thumb_k);
        }

        // Generate large display version — content-addressable (best-effort)
        let mut result_large_key = None;
        if let Some(large_data) = generate_large(&store_data) {
            let large_k = compute_key(&large_data, "image/webp");
            let large_path = self.full_path(&large_k);
            if !large_path.exists() {
                if let Some(parent) = large_path.parent() {
                    let _ = tokio::fs::create_dir_all(parent).await;
                }
                let _ = tokio::fs::write(&large_path, &large_data).await;
            }
            result_large_key = Some(large_k);
        }

        Ok(StoredImageKeys {
            storage_key: key,
            thumbnail_key: result_thumb_key,
            large_key: result_large_key,
        })
    }
    ...
    async fn retrieve_thumbnail(&self, key: &str) -> Result<(Vec<u8>, String)> {
        let thumb_key = legacy_thumbnail_key(key);
...
mod tests {
    ...
    async fn store_and_retrieve_roundtrip() {
        ...
        let keys = storage.store(data, "image/jpeg").await.unwrap();
        let (retrieved, mime) = storage.retrieve(&keys.storage_key).await.unwrap();
    ...
    async fn exists_returns_true_after_store() {
        let (storage, _dir) = make_storage();
        let keys = storage.store(b"data", "image/jpeg").await.unwrap();
        assert!(storage.exists(&keys.storage_key).await.unwrap());
    }
    ...
    async fn delete_removes_file() {
        let (storage, _dir) = make_storage();
        let keys = storage.store(b"data", "image/jpeg").await.unwrap();
        storage.delete(&keys.storage_key).await.unwrap();
        assert!(!storage.exists(&keys.storage_key).await.unwrap());
    }
    ...
    async fn store_generates_thumbnail() {
        ...
        let keys = storage.store(&png_data, "image/png").await.unwrap();
        let thumb_key = keys.thumbnail_key.expect("thumbnail should be generated");
    ...
    async fn store_generates_thumbnail_from_jpeg() {
        ...
        let keys = storage.store(&jpeg_data, "image/jpeg").await.unwrap();

        // Retrieve thumbnail and verify it decodes
        let (thumb_data, mime) = storage.retrieve_thumbnail(&keys.storage_key).await.unwrap();
    ...
    async fn retrieve_thumbnail_falls_back_to_full() {
        ...
        let keys = storage.store(data, "image/jpeg").await.unwrap();
        let (retrieved, _mime) = storage.retrieve_thumbnail(&keys.storage_key).await.unwrap();
    ...
    #[test]
    fn legacy_thumbnail_key_format() {
        assert_eq!(
            legacy_thumbnail_key("ab/abc123.jpg"),
            "ab/abc123_thumb.webp"
        );
        assert_eq!(legacy_thumbnail_key("cd/xyz.png"), "cd/xyz_thumb.webp");
    }
